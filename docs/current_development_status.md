...
- **Now: wave 2 is built.** **4b**'s diagnosis, **#17** (the `attack` to
...
| 15 | **Typed Trion (origin-keyed).** **Built 2026-08-30**, on top of #16. `TrionTokenType` (the four origins plus Wild) and a banked, squad-shared `TypedTrionReserve` on `Team`; one token per living member per turn, rolled with a Wild share and weighted toward the origins the squad's equipped abilities actually carry, over a uniform floor so every origin stays reachable. **The signature gate**: an ordinary action costs pool only, so a bad roll never stops a squad acting; a Black Trigger ability and a FAT extra action each cost one token matching the ability's origin, and a use that cannot afford both spends neither. Wired through `Battle` beside the Trion pools. Interface: origin had never been shown to a player, so it gained a label, a typed readout beside the Trion line, and a named reason when the gate blocks an ability. **Pacing held**: median 13 to 15 across four seeds, in band on all four. First-pass numbers (`TypedTrionConfig`), unpriced until wave 4. Nine tests in `test/engine/typed_trion_test.dart`. **Not built: the third gate case** (an ability's own signature effect) needs a per-ability field and belongs with wave 4's pricing; and **D5's token-draining half** needs new denial statuses, which is wave 3 content. | Built (two halves deferred) | 2 |
