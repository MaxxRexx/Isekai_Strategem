...
- **In flight: #15 reworked**, on `claude/typed-trion-rework`, not merged. The
  Trion Types mechanic was rebuilt against the Naruto-Arena research rather than
  against my guesses at it, and every ability now carries a requirement read by
  hand. One question is with the owner: the mechanic costs about three rounds of
  median pacing, and whether that trade is acceptable is theirs to call. Full
  detail in the #15 row.
- **Done: wave 2.** **4b**'s diagnosis, **#17** (the `attack` to
  `ability` rename, the Sealed recode, the Origin Lockout fix), **#16** (the
  origin re-tag rebalance), **#15** (typed Trion) and **#4** itself: the FAT
  cap, the 30-round limit with its health tiebreak, D4's range-keyed costing and
  D5's re-scale of Sapped. D1 and D3 needed no code. Verified by `dart test`
  (1098) and `flutter test` (478), and by re-running the 4b diagnosis over the
  same 800 battles. **Not playtested by a person yet**, which is the gate before
  wave 3.
- **After it:** waves 3 to 7, laid out in "The waves" directly below. Wave 3
  inherits two things #4 could not do without writing content: the positional
  traps, and a home for the four statuses nothing applies (Sealed, Sapped,
  Choked, Overcharged).
...
| **2** | ~~**4b**~~ ~~**#17**~~ ~~**#16**~~ ~~**#15**~~ ~~**#4**~~ **wave 2 complete**. | 4b went first and cleared the round limit (it found the long battles are accuracy, not economy). #4's decisions and everything the design pass pulled in are recorded in "Wave 2 decisions" above. #4 itself landed the FAT cap, the round limit, D4's range costing and D5's non-token half; D1 and D3 needed no code. Two pieces it named are wave 3 content rather than economy work: the positional traps, and a home for the three denial statuses, none of which anything applies |
| **3** | **1c**, 3b's new abilities and Side Effects, #4's positional traps, **a home for Sealed** (#12D), and **a home for the three denial statuses** (Sapped, Choked, Overcharged, left by #4's D5) | The content pass. It lands after the economy so every new ability is written against the Trion costs that will actually ship, on the rule this document has already recorded: the catalogue is priced once, not twice |
...
| **Battle Trance** | An ally-targeted support ability | FAT Chance +20 on an ally sets up next turn's burst, and it matters far more now that only one character in the squad gets FAT per turn. |
...
- **The FAT cap:** one character per squad gets FAT per turn (from the #1
  playtest). Everybody rolls; one of the winners is drawn at random and is the
  only one who has it.
...
it went to the owner rather than being re-tuned. **Answered #A: scale it to one
action.** The drain is now **12% of Capacity**, about 13 Trion a stack per turn,
so one stack costs the victim a cheap play and pays the causer the same, and the
three-stack cap costs them a turn and a half of ordinary income rather than the
battle. SPTV re-prices the status off the new number, so nothing had to be
priced by hand.

Choked and Overcharged needed no change, and D4 is why. A cost multiplier was
worth the same against everybody while every band was priced alike; now Choked's
doubling takes a 42-Trion Longshot to 84, more than two high income turns, so
choking a sniper takes their signature shot off the table while choking a
brawler is a nuisance. That asymmetry is the sub-game, and D4 produced it
without either status being touched.

**Still wave 3:** homing all three. Until the content pass gives something the
job of applying them, none of these numbers can be felt in a played battle. The
new `test/models/denial_economy_test.dart` pins the relationships (a stack
against a cheap action, three stacks against a turn's income, Choked against the
priciest ability in the game) and records the unreachability, so when the
content pass homes them the economy is already ready for it.
...
| 4 | **Trion economy.** **Decisions taken 2026-08-30 (D1 A1, D3 B, D4 A, D5 B, plus the round limit and FAT cap); full record in "Wave 2 decisions" above, which also carries the typed-Trion mechanic, the origin rebalance, the rename and the Sealed recode this item pulled in.** **Read "What 4b found" first: the round limit is cheap, and battle length is not this item's to fix.** 4b measured the 30-round limit at 3% of battles touched and 4 of 21 results reversed, so it can land as specified; it also showed the long battles are an accuracy problem, so no income number here will shorten them. **Read "Found closing out wave 1: the Levy stole a flat 20" before setting any number here**: the Levy is fixed but two questions about it are this item's (whose costliest action, and whether it caps). Also carries: a **30-round limit with a health tiebreak** (PlaySession has no round cap at all today, only the simulator does) and the FAT cap below. Screening no longer waits for this item; 1b is being built on its own. Steadier income, capacity-gated FAT, and the denial statuses becoming a real sub-game. Plus two additions agreed during the #1 playtest: **only one character per squad may have FAT per turn**. FAT still rolls per character per turn as now, and several may roll it; **one of those winners is then drawn at random** and is the only one who gets it. That character gets all of FAT unchanged, up to 3 abilities with a Reposition among them. Everyone else has an ordinary turn: one action, cooldowns untouched, no lockout. (Corrected 2026-08-31 by the owner: an earlier reading of this row capped only the extra *actions* and left the cooldown wipe with everyone who rolled, which is not the rule.) And **range becomes an input to the cost model**, with Mid Range carrying the highest cooldowns (up to 4) and Long Range the highest Trion costs, both the Loadout equip cost and the in-battle cost, up to three times the Close Range average. Each band then has an economic identity, not just a different window: Close is cheap and fast but demands you stand in the danger, Long is safe and you pay for it twice over, Mid is flexible and pays in tempo. Watch the knock-on: tripling Long Range equip costs shrinks what fits inside a Loadout's Trion Capacity, so the sniper builds may need the Capacity budget revisited in the same pass. **Also carries: more traps, designed around positional play** (see the section below). **Built 2026-08-31**, except the traps (wave 3 content) and D5's non-token half (see below). D1 and D3 needed no code: the Levy already reads the costliest ability used that turn, uncapped, and FAT extras already spend Trion. **The FAT cap** is applied at the roll (`TurnEngine.rollSquadFatTrigger`): every living member rolls, and `FatEngine.drawWinner` picks one of the winners at random. `FatEngine.rollTrigger` was split into `rollsTrigger` and `grantTrigger` for it, because granting on the roll would clear the cooldowns of characters who go on to lose the draw, and cleared cooldowns cannot be put back. Capping at the roll rather than at the action means nothing else needs a cap: only the drawn character ever has more than one use, so the queue needs no accounting of its own. **The round limit** lives in `Battle.outcome`, so Play, Quick Battle and the simulator all get it: past round 30 the battle goes to the squad ahead on total remaining health, a defeat still outranks it so a wipe reads as a wipe, and a level result at the limit is a stalemate rather than a mutual defeat because nobody was defeated in it. **D4's range costing** gave each band an economic identity (see "What D4 measured" below). **D5's non-token half** re-scaled Sapped from the brief's 25% to 12% (owner's call, #A) and left Choked and Overcharged alone, because D4's wider cost spread already made a cost multiplier worth far more against an expensive ability than a cheap one; homing all three is wave 3. | **Built** (traps are wave 3) | 2 |
...
| 15 | **Trion Types (origin-keyed).** **Built 2026-08-30, reworked 2026-09-02** after the owner reviewed it in play and the research was done properly. **What the first build got wrong:** it invented a "signature gate" where only a Black Trigger ability and a Full Arms Trigger extra action cost a type, so an ordinary turn never touched the mechanic and a squad could bank 14 Physical with nothing to spend it on; it kept a fifth kind you could hold (Wild), which is strictly better than the other four and turns the roll into a good day or a bad day; and it weighted the roll toward the squad's own origins, which defeats the question the mechanic exists to ask. **Naruto-Arena does none of those things**: four kinds and no more, Random as a *cost* any one kind can pay, an even roll of one per living member (three a turn for a full team, 1 on the opening turn), and an **Exchange** as the answer to a bad run rather than a Wild. **Now:** every ability carries a requirement of one to four, at least one slot its own origin (#2A); `TrionType` and `TrionTypeReserve` ("token" is not the owner's word and has left the codebase); the Exchange at 3, which the AI spends when it holds nothing payable. **The 61 requirements are priced individually**, with the reason beside each in the catalogue, and the price follows the worth: among the 38 abilities SPTV can price the slot count is monotonic in trigger value (checked before applying), so Shatterpoint pays three Afflict for a 10-Trion filler that is 22 acid every turn, and Longshot pays one Physical because 42 Raw Trion is already the highest price in the game. The 23 with a unique or a counter, which SPTV cannot see, are judged from the behaviour and say so. 19 pay one, 30 pay two, 12 pay three, none pay four; mean 1.89; 23 name two or more kinds. Three earlier tables were rejected on review, two of them formulas and one that let "that feels steep" override the value ordering. **Interface:** all four kinds always visible as coloured squares with counts under "Trion Types"; "Trion Available" is now "Raw Trion Reserves"; every ability description carries its requirement. **The pacing cost is real and unresolved:** median 16 rounds and 63% inside the 8-20 band, against 12 and 78% without the mechanic. Awaiting the owner's verdict on the table and on that trade. | Reworked, awaiting review | 2 |
| 16 | **Origin rebalance (re-tag).** **Built 2026-08-30.** 17 abilities re-tagged, no new content and no pricing touched: physical 23 / mental 21 / energy 11 / afflict 6 became **15 / 16 / 15 / 15**. Corrosion and bleed moved to Afflict (Acid Spray, Shatterpoint, Sunder Arms, Whirlwind Slash), as did the afflictions of the mind (Nightmare Pulse, Dread Gaze, Dread Resonance, Echoing Doubt, Soul Siphon); raw force and jammed signal moved to Energy (Psychic Scream, Scramble, Dread Wave, Mass Confusion); and four abilities that work by reading or binding rather than force took Mental (Called Shot, Curving Shot, Predictive Parry, Vow of the Duel). **That last group is what breaks the tie**: psychic was 100% Mental and Mental 100% psychic, so one tag merely restated the other; psychic is now 12 Mental / 5 Afflict / 4 Energy, and Mental sits on 4 melee and ranged abilities. Guarded by `test/content/origin_balance_test.dart`, which fails if any origin goes thin or if either tag collapses onto the other. | Built | 2 |
...
| 18 | **Two flaky tests.** Found while verifying #4, and **not caused by it**. `widget_test.dart`'s "Simulate mode drafts two AI squads and runs a real battle" and `bail_out_session_test.dart`'s "the AI does not trade a live shot for a body" each failed **once** across about twelve full `flutter test` runs, and neither reproduces in isolation (10 and 25 consecutive runs, all green). Both drive unseeded randomness: the first drafts two random squads, the second runs a real AI turn whose FAT roll and target tie-breaks are live. A rate of roughly 1 in 40 fits what was seen. `PlaySession.start` already takes a `turnEngine`, so a seeded `FatEngine` is the likely seam for the second; the first needs a seed on the Simulate screen's draft. Small, and worth doing before anyone trusts a green suite as a gate. | Found, not fixed | 4 |
| 19 | **The long-battle diagnosis blamed the wrong gate.** Found while measuring #15's rework. Its idle-reason question ran through `canUseAbility`, which now answers false for the Trion Type gate as well, so every turn blocked on types was counted as blocked on cooldowns: it reported 17 cooldown idles where the truth was 1895 type idles. It asks the two separately now, via `canUseAbilityIgnoringTrionTypes`, and prints a "no types" column. Same class of defect as the measurement flaw 4b turned up, and the same lesson: a tool that routes a diagnostic question through a gameplay predicate starts lying the moment the predicate grows a new reason. | Fixed | - |
